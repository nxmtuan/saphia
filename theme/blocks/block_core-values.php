<section class="lg:my-[150px] my-[60px]" data-component="block_core-values">
	<div class="container">
		<div
			...
			<?php
			$count = count(get_field('list_content'));
			if ( have_rows( 'list_content' ) ) : ?>
				<div class="flex-1 grid grid-cols-2 xl:gap-7 gap-2.5">
					...
					while ( have_rows( 'list_content' ) ) :
						...
						?>
						<div
							class="xl:px-11 xl:pt-10 xl:pb-8 p-5 bg-[#FFF3E7] lg:rounded-[30px] rounded-[20px] overflow-hidden <?= $i == 2 && $count == 5 || $i == 4 && $count == 5 ? 'translate-y-[90px]' : '' ?> <?= $i == 1 && $count == 4 || $i == 3 && $count == 4 ? 'translate-y-[90px]' : '' ?>">
